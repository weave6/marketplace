...
export function runExchangeTests(setupExchange: SetupExchangeFunction, publicMutableMethods: string[]) {
  describe("Exchange", function () {
    ...
    describe("validateOrderParameters", function () {
      ...
      it("trader is zero", async () => {
        ...
        expect(await exchange.validateOrderParameters(order.parameters, orderHash)).to.equal(false);
      });
      ...
      it("expiration time > now and listing time < now", async () => {
        ...
        expect(await exchange.validateOrderParameters(order.parameters, orderHash)).to.equal(true);
      });
      it("expiration time < now and listing time < now", async () => {
