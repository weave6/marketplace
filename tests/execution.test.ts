...
import { CheckBalances, GenerateOrder, signBulk } from "./exchange";
import { eth, Order } from "./exchange";
import { Side, ZERO_ADDRESS, getListingTime, getExpirationTime } from "./exchange/utils";
...
export function runExecuteTests(setupTest: any) {
  return async () => {
    ...
    describe("sigle execution", () => {
      beforeEach(async () => {
        ...
        sell = generateOrder(alice, {
          side: Side.Sell,
          tokenId,
          listingTime: await getListingTime(),
          expirationTime: await getExpirationTime(),
        });
        ...
        otherOrders = [
          generateOrder(alice, { salt: 1, listingTime: await getListingTime(), expirationTime: await getExpirationTime() }),
          generateOrder(alice, { salt: 2, listingTime: await getListingTime(), expirationTime: await getExpirationTime() }),
          generateOrder(alice, { salt: 3, listingTime: await getListingTime(), expirationTime: await getExpirationTime() }),
        ];
      ...
      it("should revert with ERC20 not WETH", async () => {
        ...
        sellInput = await sell.pack();
        buyInput = await buy.pack();
        console.log("sellInput:", sellInput, "\n buyInput:", buyInput, "weth:", weth.address);
        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.revertedWith("Invalid payment token");
      });
      it("should revert if Exchange is not approved by ExecutionDelegate", async () => {
        ...
        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.revertedWith("Contract is not approved to make transfers");
        await executionDelegate.approveContract(exchange.address);
      });
      it("should succeed is approval is given", async () => {
        await executionDelegate.approveContract(exchange.address);
        buyInput = await buy.pack();
        const tx = await waitForTx(exchange.connect(bob).execute(sellInput, buyInput));
      ...
      it("should revert if user revokes approval from ExecutionDelegate", async () => {
        ...
        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.revertedWith("User has revoked approval");
        await executionDelegate.approveContract(exchange.address);
      });
      ...
      it("buyer sends tx with ETH", async () => {
        ...
        const tx = await waitForTx(exchange.connect(bob).execute(sellInput, buyInput, { value: price }));
        const gasFee = tx.gasUsed.mul(tx.effectiveGasPrice);
      ...
      it("seller tx fails with ETH", async () => {
        ...
        await expect(exchange.connect(alice).execute(sellInput, buyInput)).to.be.reverted;
      });
      it("seller sends tx with WETH", async () => {
        sellInput = await sell.pack();

        const tx = await waitForTx(exchange.connect(alice).execute(sellInput, buyInput));
        const gasFee = tx.gasUsed.mul(tx.effectiveGasPrice);
      ...
      it("can cancel bulk listing", async () => {
        ...
        await expect(exchange.execute(sellInput, buyInput)).to.be.revertedWith("Sell has invalid parameters");
      });
      ...
      it("should not cancel if not user", async () => {
        await expect(exchange.connect(alice).cancelOrder(buy.parameters)).to.be.reverted;
      });
      ...
      it("should not match with invalid signatures buy", async () => {
        ...
        await expect(exchange.execute(sellInput, buyInput)).to.be.revertedWith("Buy failed authorization");
      });
      it("should revert if orders cannot be matched", async () => {
        sell.parameters.price = BigNumber.from("1");
        sellInput = await sell.pack();

        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.revertedWith("Orders cannot be matched");
      });
      it("should revert policy is not whitelisted", async () => {
        ...
        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.revertedWith("Policy is not whitelisted");
      });
      it("should revert if buyer has insufficient funds ETH", async () => {
        ...
        await expect(exchange.connect(bob).execute(sellInput, buyInput)).to.be.reverted;
      });
      ...
      it("should revert if closed", async () => {
        ...
        await expect(exchange.execute(sellInput, buyInput)).to.be.revertedWith("Closed");
      });
    ...
    describe("bulk execution", () => {
      ...
      it("execute bulk order", async () => {
        let executions = [];
        sellInput = sell.packBulk(otherOrders);
        buyInput = buy.pack();
        /* for(let i = 0; i< otherOrders.length; i++){
          otherOrders[i].parameters.trader = bob.address;
          otherOrders[i].parameters.side = Side.Buy;
          otherOrders[i].parameters.fees = []
        }
        buyInput = buy.packBulk(otherOrders); */
        const tx = await waitForTx(exchange.connect(bob).bulkExecute([{ sell: sellInput, buy: buyInput }]));
        const gasFee = tx.gasUsed.mul(tx.effectiveGasPrice);

        expect(await mockERC721.ownerOf(buy.parameters.tokenId)).to.be.equal(bob.address);
        await checkBalances(
          aliceBalance,
          aliceBalanceWeth.add(priceMinusFee),
          bobBalance.sub(gasFee),
