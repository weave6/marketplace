...
export function packSignature(signature: Signature): string {
  return ethers.utils.defaultAbiCoder.encode(["uint8", "bytes32", "bytes32"], [signature.v, signature.r, signature.s]);
}
...
export function hash(parameters: any, exchange: any): string {
  ...
  return `0x${eip712Hash(
    {
      ...
      domain: {
        ...
        version: "1.0",
        chainId: 1,
        verifyingContract: typeof exchange == typeof Contract ? exchange.address : exchange,
      },
      message: parameters,
    },
...
export function hashWithoutDomain(parameters: any): string {
  ...
  return `0x${hashStruct(
    ...
    SignTypedDataVersion.V4, // version
  ).toString("hex")}`;
}
