...
export class Order {
  ...
  constructor(user: any, parameters: OrderParameters, admin: any, exchange: any) {
    this.user = user;
  ...
  async pack(options: { signer?: Signer; oracle?: Signer; blockNumber?: number; signatureVersion?: number } = {}) {
    ...
    return {
      ...
      signatureVersion: options.signatureVersion || 0,
      blockNumber: options.blockNumber || (await ethers.provider.getBlock("latest")).number,
    };
  }
  ...
  async packBulkNoOracleSig(otherOrders: OrderParameters[]) {
    const { path, r, v, s } = await signBulk([this.parameters, ...otherOrders], this.user, this.exchange);
    return {
      order: this.parameters,
      r,
      v,
  ...
  async packBulk(otherOrders: Order[]) {
    const { path, r, v, s } = await signBulk([this.parameters, ...otherOrders.map((_) => _.parameters)], this.user, this.exchange);
    const oracleSig = await oracleSign(this.parameters, this.admin, this.exchange, (await ethers.provider.getBlock("latest")).number);
    return {
      order: this.parameters,
      r,
      v,
...
export async function getListingTime() {
  return (await ethers.provider.getBlock("latest")).timestamp - 15 * 60 * 1000;
}
