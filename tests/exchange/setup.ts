...
export async function deploy(hre: any, name: string, calldata: any = [], options: any = {}) {
...
export type SetupExchangeFunction = (opts: SetupExchangeOpts) => Promise<SetupExchangeResult>;
...
async function setupRegistry(
  ...
  await weth.connect(bob).approve(executionDelegate.address, eth("10000000000000"));
  await weth.connect(alice).approve(executionDelegate.address, eth("1000000000000"));
}
...
async function setupMocks(alice: any, bob: any) {
  ...
  await weth.mint(bob.address, eth("10000"));
  await weth.mint(alice.address, eth("10000"));
...
export async function setupTest({ price, feeRate, setupExchange }: SetupTestOpts): Promise<SetupTestResult> {
  ...
  // log("admin:", admin, "alice:", alice, "bob:", bob, "thirdParty:", thirdParty)

  const { weth, mockERC721, mockERC1155, tokenId } = await setupMocks(alice, bob);
  const { exchange, executionDelegate, matchingPolicies } = await setupExchange({
    admin,
    weth,
  });
  await setupRegistry(alice, bob, mockERC721, mockERC1155, weth, executionDelegate);

  const checkBalances = async (aliceEth: any, aliceWeth: any, bobEth: any, bobWeth: any, feeRecipientEth: any, feeRecipientWeth: any) => {
    ...
    expect(await (admin.provider as ethers.providers.Provider).getBalance(thirdParty.address)).to.be.equal(feeRecipientEth);
