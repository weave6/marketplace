...
import "./interfaces/IWeave6Exchange.sol";
...
/**
 * @title Weave6Exchange
 * @dev Core Weave6 exchange contract
 */
contract Weave6Exchange is IWeave6Exchange, ReentrancyGuarded, EIP712, OwnableUpgradeable, UUPSUpgradeable {
  /* Auth */
  ...
  /* Constants */
  string public constant NAME = "Weave6 Exchange";
  string public constant VERSION = "1.0";
  uint256 public constant INVERSE_BASIS_POINT = 10_000;
  uint256 private constant MAX_FEE_RATE = 500;
  ...
  function initialize(IExecutionDelegate _executionDelegate, IPolicyManager _policyManager, address _oracle, uint _blockRange) external initializer {
    ...
    DOMAIN_SEPARATOR = _hashDomain(EIP712Domain({name: NAME, version: VERSION, chainId: block.chainid, verifyingContract: address(this)}));
    ...
    reentrancyGuard // move re-entrancy check for clarity
  {
    ...
    _executeFundsTransfer(sell.order.trader, buy.order.trader, sell.order.paymentToken, sell.order.fees, buy.order.fees, price);
    _executeTokenTransfer(sell.order.collection, sell.order.trader, buy.order.trader, tokenId, amount, assetType);

    emit OrdersMatched(sell.order.listingTime <= buy.order.listingTime ? sell.order.trader : buy.order.trader, sell.order.listingTime > buy.order.listingTime ? sell.order.trader : buy.order.trader, sell.order, sellHash, buy.order, buyHash);
  }
  ...
  function setTokenWhitelisted(address _token) external onlyOwner {
    require(_token != address(0), "Address cannot be zero");
    whitelistedTokens[_token] = true;
    emit TokenWhitelisted(_token);
  }
  function removeTokenWhitelisted(address _token) external onlyOwner {
  ...
  function _validateSignatures(Input calldata order, bytes32 orderHash) internal view returns (bool) {
    ...
    if (!_validateUserAuthorization(orderHash, order.order.trader, order.v, order.r, order.s, order.signatureVersion, order.extraSignature)) {
      return false;
    }
   ...
   */
    function _validateUserAuthorization(bytes32 orderHash, address trader, uint8 v, bytes32 r, bytes32 s, SignatureVersion signatureVersion, bytes calldata extraSignature) internal view returns (bool) {
    bytes32 hashToSign;
   ...
   */
  function _validateOracleAuthorization(bytes32 orderHash, SignatureVersion signatureVersion, bytes calldata extraSignature, uint256 blockNumber) internal view returns (bool) {
   ...
   * @param price price
   */
  function _executeFundsTransfer(address seller, address buyer, address paymentToken, Fee[] calldata sellerFees, Fee[] calldata buyerFees, uint256 price) internal {
   ...
   */
  function _transferFees(Fee[] calldata fees, address paymentToken, address from, uint256 price, bool protocolFee) internal returns (uint256) {
    uint256 totalFee = 0;
  ...
  function _transferTo(address paymentToken, address from, address to, uint256 amount) internal {
    ...
    if (paymentToken == address(0)) {
      ...
      bool success = IWeave6Pool(POOL).transferFrom(from, to, amount);
      require(success, "Pool transfer failed");
    } else if (whitelistedTokens[paymentToken]) {
      /* Transfer funds in WETH or whitelisted ERC20 token. */
   ...
   */
  function _executeTokenTransfer(address collection, address from, address to, uint256 tokenId, uint256 amount, AssetType assetType) internal {
